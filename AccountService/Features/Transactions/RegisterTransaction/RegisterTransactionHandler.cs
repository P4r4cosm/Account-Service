...
public class RegisterTransactionHandler(IAccountRepository accountRepository, 
...
{
    public async Task<MbResult<TransactionDto>> Handle(RegisterTransactionCommand request, CancellationToken cancellationToken)
    {
        var account = await accountRepository.GetByIdAsync(request.AccountId, cancellationToken);
        if (account is null)
            return MbResult<TransactionDto>.Failure(MbError.Custom("Account.NotFound", $"Счёт {request.AccountId} не найден."));
        
        if (account.CloseDate.HasValue)
            return MbResult<TransactionDto>.Failure(MbError.Custom("Account.Closed", "Операции по закрытому счёту невозможны."));
        ...
        if (newTransaction.Type == TransactionType.Debit && account.Balance < request.Amount)
            return MbResult<TransactionDto>.Failure(MbError.Custom("Transaction.InsufficientFunds", "Недостаточно средств на счёте для списания."));
        ...
        catch (DbUpdateConcurrencyException ex)
        {
            ...
            logger.LogWarning(ex, "Конфликт параллельного доступа при регистрации транзакции для счёта {AccountId}", request.AccountId);
            return MbResult<TransactionDto>.Failure(MbError.Custom("Account.Conflict", "Не удалось выполнить операцию, данные счёта были изменены. Попробуйте снова."));
        }
        catch (Exception ex) // Лучше ловить более конкретный DbUpdateException
        {
            await unitOfWork.RollbackTransactionAsync(cancellationToken); // Откатываем явную транзакцию
            logger.LogError(ex, "Ошибка базы данных при регистрации транзакции для счёта {AccountId}", request.AccountId);
            return MbResult<TransactionDto>.Failure(MbError.Custom("Database.Error", "Произошла ошибка при сохранении данных."));
        }
