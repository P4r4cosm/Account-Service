...
*   Асинхронное взаимодействие с другими системами через брокер сообщений.
...
*   **RabbitMQ:** в качестве брокера сообщений для асинхронного взаимодействия между сервисами.
...
*   **Transactional Outbox:** Для обеспечения надежной доставки доменных событий используется паттерн "Транзакционный исходящий ящик". Запись в БД и сообщение для брокера сохраняются в одной атомарной транзакции. Отдельный фоновый процесс (Hangfire job) отвечает за гарантированную доставку сообщений в RabbitMQ.
*   **Idempotent Consumer (Inbox):** На стороне потребителей реализован паттерн "Входящий ящик" для обеспечения идемпотентности. Сервис отслеживает ID обработанных сообщений, что предотвращает повторное выполнение бизнес-логики при дублирующихся доставках от брокера.
*   **Отказоустойчивость:** Взаимодействие с RabbitMQ и базой данных обернуто в политики повторных попыток (`Polly`) с экспоненциальной выдержкой для устойчивости к временным сбоям сети или недоступности сервисов.
## 🔐 Аутентификация
...
Сервис использует RabbitMQ для публикации и потребления доменных событий.
...
| :--- | :--- | :--- |
| `AccountOpened` | `account.opened` | Открыт новый счёт клиента. |
| `AccountClosed` | `account.closed` | Счёт был закрыт. |
| `AccountReopened` | `account.reopened` | Ранее закрытый счёт был снова открыт. |
| `AccountOwnerChanged` | `account.ownerChanged` | Изменился владелец счёта. |
| `AccountInterestRateChanged` | `account.rateChanged` | Изменилась процентная ставка по вкладу. |
| `MoneyCredited` | `money.credited` | Произошло пополнение счёта. |
| `MoneyDebited` | `money.debited` | Произошло списание со счёта. |
| `TransferCompleted` | `money.transfer.completed` | Успешно завершен перевод между счетами. |
| `InterestAccrued` | `money.interest.accrued`| Начислены проценты по вкладу. |

### Потребляемые события

| Событие | Очередь | Routing Key | Описание |
| :--- | :--- | :--- | :--- |
| `ClientBlocked` | `account.antifraud` | `client.blocked` | Получено от Antifraud. Все счета клиента замораживаются (запрет расходных операций). |
| `ClientUnblocked`| `account.antifraud` | `client.unblocked`| Получено от Antifraud. Со счетов клиента снимается заморозка. |

Полная документация по форматам событий и примеры JSON доступны в Swagger в разделе **"События v1"**.
...
2.  **Запустите контейнеры:**
    ...
    ```
3.  **Дождитесь запуска всех сервисов.** Вы можете отслеживать логи командой `docker-compose logs -f`.
4.  **Откройте UI в браузере:**
    *   **API сервиса (Swagger):** `http://localhost/swagger`
